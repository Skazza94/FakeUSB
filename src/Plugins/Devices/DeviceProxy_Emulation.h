...
private:
	...
	std::map<__u8, std::list<std::pair<__u8 *, __u64>> *> * endPoint2packetBuffer;
...
public:
	...
	void send_data(__u8, __u8, __u16, __u8 *, __u64);
	void receive_data(__u8, __u8, __u16, __u8 **, __u64 *, int timeout = 500);
	...
	std::list<std::pair<__u8 *, __u64>> * getPacketBufferForEndpoint(__u8);
	void setPacketBufferForEndpoint(std::list<std::pair<__u8 *, __u64>> *, __u8);
